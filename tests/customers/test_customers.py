...
def test_customer_list_search_and_status_filter(authenticated_page):
    users = UsersPage(authenticated_page)
    users.open_list(); users.assert_list_loaded(); users.search("automation-probe")
...
def test_customer_add_form_required_validation_without_submit(authenticated_page):
    users = UsersPage(authenticated_page); users.open_list(); users.open_add(); users.assert_add_form()
...
def test_customer_status_tabs_are_available(authenticated_page):
    users = UsersPage(authenticated_page); users.open_list()
